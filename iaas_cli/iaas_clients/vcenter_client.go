...
func (c VcenterClient) UploadArtifact(vmInventoryPath, artifact, destination string) error {

	errCode := c.Runner.Run([]string{"guest.upload", "-u", c.credentialUrl, "-l", "user:pass", "-vm", vmInventoryPath, artifact, destination})
	if errCode != 0 {
