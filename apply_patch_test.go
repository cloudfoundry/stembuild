...
// func TestErrorStringWithBadPatchInfo(t *testing.T) {
// 	badPatchInfo := ApplyPatchInfo{}

// 	err := validatePatchInfoAttributes(badPatchInfo)
...
// 	if !strings.Contains(errString, "version") {
// 		t.Error("expected error string to contain 'version', but it did not")
// 	}

// 	if !strings.Contains(errString, "vhd_file_name") {
// 		t.Error("expected error string to contain 'vhd_file_name', but it did not")
// 	}

// 	if !strings.Contains(errString, "patch_file") {
// 		t.Error("expected error string to contain 'patch_file', but it did not")
// 	}
// }
...
var _ = Describe("Apply Patch", func() {
	Context("CopyInto", func() {
		...
		Context("DeltaFile", func() {
			Context("when src specifies an DeltaFile and dest does not", func() {
				BeforeEach(func() {
					src.DeltaFile = fmt.Sprintf("blackberry%d", rand.Intn(2000))
				})

				It("copies src.DeltaFile into dest.DeltaFile", func() {
					Expect(dest.DeltaFile).To(Equal(src.DeltaFile))
				})
			})

			Context("when src specifies a DeltaFile and dest specifies a DeltaFile", func() {
				var expectedDestDeltaFile string

				BeforeEach(func() {
					src.DeltaFile = fmt.Sprintf("blackberry%d", rand.Intn(2000))
					dest.DeltaFile = fmt.Sprintf("blackberry%d", rand.Intn(2000))
					expectedDestDeltaFile = dest.DeltaFile
				})

				It("retains dest.DeltaFile's original value", func() {
					Expect(dest.DeltaFile).To(Equal(expectedDestDeltaFile))
				})
			})

			Context("when src specifies a DeltaFile and dest specifies a DeltaFile", func() {
				It("should do nothing", func() {
					Expect(dest.DeltaFile).To(BeEmpty())
				})
			})
		})
